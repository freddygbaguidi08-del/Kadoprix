  ...
  for (const r of ROUTES) {
    try {
      ...
      await upsertDeal({
        ...
        prix_barre: null,
        devise: 'EUR',
        image: `https://kadoprix.vercel.app/api/voyage-img/${r.from}-${r.to}?to=${encodeURIComponent(r.nomTo)}&code=${r.to}`,
  ...
  for (const v of VILLES_HOTELS) {
    try {
      ...
      await upsertDeal({
        ...
        prix_barre: null,
        devise: 'EUR',
        image: `https://kadoprix.vercel.app/api/voyage-img/hotel-${v.iata}?to=${encodeURIComponent(v.nom)}&code=${v.iata}`,
